...
def c_h(n_gpu):
	...
	client = cah.init(
	    ...
	    device_id="machine1:0" # cuda:0 at machine1 (I'd recommend zero-padding the gpu number if gpus > 9).
	...
	model_T5 = SimpleT5()
	if torch.cuda.is_available():
		model_T5.load_model("t5","./simplet5-epoch-2-train-loss-0.2212-val-loss-0.2188", use_gpu=True) #outtest/simplet5-epoch-2-train-loss-0.1273-val-loss-0.1379", use_gpu=False) #repair1/simplet5-epoch-1-train-loss-0.2125-val-loss-0.2048", use_gpu=True)
	else:
		model_T5.load_model("t5","./simplet5-epoch-2-train-loss-0.2212-val-loss-0.2188", use_gpu=False) #outtest/simplet5-epoch-2-train-loss-0.1273-val-loss-0.1379", use_gpu=False) #repair1/simplet5-epoch-1-train-loss-0.2125-val-loss-0.2048", use_gpu=True)
	...
	while client.jobCount() > 0 and not client.shouldDie():
	    ...
	    for i, d in enumerate(dataset):
	        ...
	        # print(d)
	        start=time.time()
	        raw_image = Image.open(io.BytesIO(d['jpg']))
	    ...
	    for abc in range(100):
	        resp = upload('./c_h/captioning_result_'+url.split("/")[-1].split(".")[0]+'.json')
	        if resp == 5888:
...
while True:
	...
	print(j)
	j+=1
	for i in test: #range(n_gpus):
		
	    print(i)

	    p = multiprocessing.Process(target=c_h, args=(i, ))
	    jobs.append(p)
	    p.start()
